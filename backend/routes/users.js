...
router.get('/search', authMiddleware, async (req, res) => {
  try {
    ...
    let query = 'SELECT id, name, location, profile_photo_url, availability FROM users WHERE id != $1';
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const userResult = await pool.query(
      'SELECT id, name, email, location, profile_photo_url, availability, created_at FROM users WHERE id = $1',
      [userId]
    );
